...
class KeyStatusCache:
    ...
    def _cache_key(self, provider: str) -> str:
        return f"keypool:active:{provider}"

    async def get_active(self, provider: str) -> list[APIKeyDTO] | None:
        raw = await self._redis.get(self._cache_key(provider))
        if raw is None:
            return None
        data = json.loads(raw)
        return [APIKeyDTO.model_validate(item) for item in data]

    async def set_active(self, provider: str, keys: list[APIKeyDTO]) -> None:
        payload = json.dumps([key.model_dump(mode="json") for key in keys])
        await self._redis.set(self._cache_key(provider), payload, ex=self._ttl)

    async def invalidate(self, provider: str) -> None:
        await self._redis.delete(self._cache_key(provider))
