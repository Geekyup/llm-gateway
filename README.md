 ...
 /v1/chat/completions  ──────────────►  Селектор ключей (Postgres)
  ...
  Адаптер провайдера (Gemini / OpenRouter) ◄────┘
        ...
        └─ исчерпан (403/402) → ключ → EXHAUSTED, retry на следующем ключе
                   ...
                   Redis (pub/sub + ограниченный список)
                              │
                    ┌─────────┴─────────┐
                    ▼                   ▼
          Живой монитор (SSE)   Почасовые графики usage
```
...
- **ARQ** гоняет запланированные housekeeping-задачи (снятие истёкших
  ...
  ключей) вместо самопального cron+скрипта — так retry/расписание
  обрабатываются полноценной очередью задач, уже подключённой к тому же
  Redis.
- **Мониторинг сознательно построен только на Redis.** Живые события идут
  через pub/sub-канал (fan-out на все текущие подключённые дашборды) плюс
  ограниченный список на пользователя (чтобы дашборд, открытый уже после
  того, как прошёл трафик, не смотрел на пустой экран). Отдельной
  time-series таблицы для этого нет.

## Компромиссы

- **История использования на Redis вместо выделенной таблицы событий.**
  Графики почасового usage/токенов читаются из того же ограниченного
  списка `monitoring:requests:history:{user_id}`, который использует живой
  монитор — а не из персистентного, неограниченного лога. Это осознанный
  выбор в пользу простоты: он избавляет от второго пути записи и
  растущей таблицы только ради графика в UI. Реальная цена честно
  задокументирована прямо в коде — на активном аккаунте самые ранние часы
  насыщенного дня уже могут "укатиться" из ограниченного списка к моменту,
  когда смотришь на график. Если аналитика использования когда-нибудь
  должна станет источником истины (биллинг, долгосрочные тренды) — это
  первое, что стоит заменить на настоящую таблицу событий.
- **Best-effort учёт токенов.** Извлечение usage из ответа провайдера
  обёрнуто в try/except, который никогда не блокирует сам ответ прокси —
  если провайдер поменяет формат ответа или поле отсутствует, запрос всё
  равно успешно доходит до вызывающей стороны, а подсчёт токенов для этого
  события просто пропускается, вместо того чтобы ронять весь запрос
  из-за детали учёта.
- **Шифрование вместо ключа-в-переменной-окружения.** Ключи провайдеров
  хранятся зашифрованными (Fernet) в Postgres, а не передаются как
  переменные окружения — так ключи можно добавлять/ротировать/отключать
  на пользователя прямо из дашборда без передеплоя, ценой того, что
  управление `ENCRYPTION_KEY` становится отдельной операционной задачей.
- **Cooldown vs exhausted — эвристика, а не гарантия.** `429` трактуется
  как временное состояние (таймер cooldown, автовосстановление), а
  специфичный для провайдера сигнал "квота закончилась" (например, `402`
  у OpenRouter) — как требующий вмешательства человека. Это соответствует
  реальному поведению этих провайдеров, но остаётся решением для каждого
  конкретного провайдера, закодированным в `is_key_exhausted`, а не
  универсальной гарантией API.